 ...
 * 5.遍历到 arr[3]==4，当前 qmax的队尾下标为 2，又有 arr[2]＞arr[3]，所以将下标 3放入qmax尾部，qmax={2，3}。窗口arr[1..3]出现，当前qmax队头的下标为2，这个下标还没有过期，所以窗口arr[1..3]的最大值为arr[2]（即5）。
 * 6.遍历到 arr[4]==3，当前 qmax的队尾下标为 3，又有 arr[3]＞arr[4]，所以将下标 4放入qmax尾部，qmax={2，3，4}。窗口arr[2..4]出现，当前qmax队头的下标为2，这个下标还没有过期，所以窗口arr[2..4]的最大值为arr[2]（即5）。
 ...
 * 9.遍历到 arr[7]==7，当前 qmax的队尾下标为 6，又有 arr[6]＜=arr[7]，所以将下标 6从qmax的尾部弹出，qmax变为{}。将下标7放入qmax，qmax={7}。窗口arr[5..7]出现，当前qmax队头的下标为7，这个下标没有过期，所以窗口arr[5..7]的最大值为arr[7]（即7）。
...
public class Demo07 {
    ...
    public static int[] getMaxWindow(int[] arr,int w){
        ...
        int[] res=new int[arr.length-w+1];//一共会产生arr.length-w+1个窗口最大值
        int index=0;
        for (int i=0;i<arr.length;i++){
            while (!qmax.isEmpty() && arr[qmax.peekLast()]<=arr[i]){
                qmax.pollLast();//弹出最后插入的
            }
            qmax.addLast(i);
            if (qmax.peekFirst()== i-w){//队头已过期
                qmax.pollFirst();
            }
            if (i>= w-1){
